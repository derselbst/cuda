...
\section{Datenmodell}
Um eine effiziente Verarbeitung auf der Grafikkarte zu erreichen, muessen die Datensaetze in geeigneter Weise strukturiert werden.
Dazu muss entschieden werden, ob sie Daten als \gls{aos} oder als \gls{soa} uebergeben werden. Nachfolgend werden diese Ansaetze diskutiert.
...
Es muessen somit weitere Cachlines geladen werden, was schlimmsten falls zu einer Ausserialisierung der Threads fuehrt.
...
Es empfiehlt sich daher die Datensaetze column major im Speicher abzulegen, wie in Listing \ref{dings} dargestellt. Die i-te Zeile des Speichers enthaelt somit die i-ten Messwerte aller Datensaetze. Da jeder Thread einen Datensatz bearbeitet, koennen alle 32 Threads eines Warps durch eine Cacheline mit Daten versorgt werden.


\paragraph{\gls{soa}}
Bei NVidia Grafikkarten vor der Pascal Architektur erhält jeder Thread durch einen Lesevorgang ein 32 bit Wort aus der Cacheline\footnote{http://docs.nvidia.com/cuda/pascal-tuning-guide/index.html#shared-memory-bandwidth}. tuple\_t hat jedoch eine Größe von 8 Bytes. 
Es braucht also zwei Lesevorgänge um alle Threads eines Warps mit Daten zu versorgen.
Um einen Performanceverlgeich zu der aelteren Kepler Architektur zu erhalten, wird daher auch eine \gls{soa} Variante, wie in Listing \ref{dings} dargestellt, implementiert.
...
Zwar hat tuple\_t nun eine Groesse von 16 Byte, da es zwei 8 Byte Zeiger enthaelt. Dieses Element muss jedoch nur einmalig gelesen werden, da die enthaltenen Zeiger von den N Threads gemeinsam verwendet werden koennen.
